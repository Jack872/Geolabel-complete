...
from trainers import train_torch_model  # 移除 sklearn 相关
...
from utils_db import connect_db, fetch_labels_from_db, match_typeid_to_name, save_model_to_db
# from utils_yolo import create_yolo_data_yaml, create_yolo_dataset, filter_original_labels, process_yolo_results
from utils_yolo import create_Multi_yolo_data_yaml, create_multi_image_yolo_dataset  # 使用新函数
...
def train_Multi_function(argv=None):
    ...
    if MODEL_TYPE in ["light_unet", "unet", "fast_scnn", "deeplab"]:
        ...
        temp_yolo_data_dir = os.getenv("YOLO_DATASET_DIR", "/opt/geolabel/cache/yolo-datasets")
        os.makedirs(temp_yolo_data_dir, exist_ok=True)

        # 如果你想每次清空旧数据（模拟临时目录行为），可以加：
        # shutil.rmtree(temp_yolo_data_dir, ignore_errors=True)
        # os.makedirs(temp_yolo_data_dir, exist_ok=True)
        images_base_dir, labels_base_dir, *_ = create_multi_image_yolo_dataset(
            all_image_paths,
            all_labels_data_per_image,
            temp_yolo_data_dir,
        ...
        model = YOLO("yolo11n-obb.pt") # 或者 "yolov8n-obb.pt" 等
        ...
        model.train(
            ...
            # 新增参数
            batch=4,  # 降低批次大小 (如果4不报错，后续可以尝试改成 8)
            mosaic=0.0,  # 彻底关闭马赛克增强 (防止 DataLoader 同时读入4张巨幅大图)
            cache=False,  # 确保不将图像缓存在内存中
        )
        model = None # 释放显存
        torch.cuda.empty_cache()
        ...
        mapping_parts = [
            f"{type_id_to_name.get(type_id, f'未知类别-{type_id}')}:  {class_id}"
