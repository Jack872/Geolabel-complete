    ...
    try:
        with conn.cursor() as cursor:
            query = f"""
                SELECT
                  CASE
                    WHEN task_source = 'local' AND local_image_path IS NOT NULL AND local_image_path <> ''
                      THEN local_image_path
                    ELSE map_server
                  END AS map_path
                FROM {table_name}
                WHERE task_id = %s
...
def fetch_model_by_id(conn, model_id, table_name="model"):
    """
    从数据库中根据模型ID查找单条模型记录。
    ...
    try:
        ...
        print(f"Error fetching model by id from database: {e}")
        return {}
    except Exception as e:
        print(f"An unexpected error occurred: {e}")
        return {}
...
def fetch_user_models(conn, user_id, table_name="model"):
    """
    从数据库中查询指定用户可访问的所有训练模型。
    ...
    Returns:
        返回模型列表，每个模型是一个字典。
        如果未找到记录、conn为None或发生数据库错误，则返回空列表 []。
    """
    if conn is None:
        return []
    ...
    try:
        with conn.cursor() as cursor:
            # 查询用户拥有的模型（暂不考虑共享模型）
            query = f"""
                SELECT id, model_name, model_type, task_type, path, user_id, status
                FROM {table_name}
                WHERE user_id = %s
                ORDER BY id DESC
            """
        ...
        print(f"Error fetching user models from database: {e}")
        return []
    except Exception as e:
        print(f"An unexpected error occurred: {e}")
        return []
...
def delete_latest_prompt_db(conn, task_id, user_id, table_name="mark"):
    ...
    try:
        ...
        # 提交事务
        conn.commit()
        print(f"已清理用户 {user_id} 在任务 {task_id} 下的最新提示要素。")

    except Exception as e:
        # 出错回滚
        print(f"Error deleting latest prompt from database: {e}")
        conn.rollback()
