...
//   此参数待定，boolean isAutoLogin
    ...
    public Result updateUser(@RequestBody Map<String, Object> map) {
        ...
        boolean containsTeamId = map.containsKey("teamId");
        Integer requestedTeamId = null;
        if (containsTeamId) {
            Object teamIdValue = map.get("teamId");
            if (ObjectUtil.isNotNull(teamIdValue) && StrUtil.isNotBlank(ObjectUtil.toString(teamIdValue))) {
                requestedTeamId = Integer.valueOf(ObjectUtil.toString(teamIdValue));
            }
        }
        if (ObjectUtil.isNotNull(userid)) {
            Integer userIdValue = Integer.valueOf(userid);
            SysUser targetUser = SysUserService.findByUserId(userIdValue);
            if (ObjectUtil.isNull(targetUser)) {
                return ResultGenerator.getFailResult("用户不存在");
            }

            Integer effectiveTeamId = targetUser.getTeamId();
            if (containsTeamId) {
                LoginUser loginUser = (LoginUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
                SysUser currentUser = loginUser.getSysUser();
                boolean isCurrentUserAdmin = ObjectUtil.isNotNull(currentUser) && Integer.valueOf(1).equals(currentUser.getIsadmin());
                if (!isCurrentUserAdmin) {
                    return ResultGenerator.getFailResult("仅管理员可以分配团队");
                }
                if (Integer.valueOf(1).equals(targetUser.getIsadmin())) {
                    return ResultGenerator.getFailResult("暂不支持修改管理员团队");
                }
                Integer currentAdminTeamId = currentUser.getTeamId();
                if (ObjectUtil.isNotNull(requestedTeamId)) {
                    TeamTable team = teamService.getById(requestedTeamId);
                    if (ObjectUtil.isNull(team)) {
                        return ResultGenerator.getFailResult("目标团队不存在");
                    }
                    if (ObjectUtil.isNull(currentAdminTeamId) || !requestedTeamId.equals(currentAdminTeamId)) {
                        return ResultGenerator.getFailResult("只能分配到当前管理员所属团队");
                    }
                    effectiveTeamId = requestedTeamId;
                } else {
                    if (ObjectUtil.isNotNull(targetUser.getTeamId())) {
                        if (ObjectUtil.isNull(currentAdminTeamId) || !currentAdminTeamId.equals(targetUser.getTeamId())) {
                            return ResultGenerator.getFailResult("只能移出当前管理员所属团队的成员");
                        }
                    }
            ...
            boolean iaUpdateUser = SysUserService.updateUser(userIdValue, username, Integer.valueOf(isadmin), effectiveTeamId);
            if (iaUpdateUser) {
