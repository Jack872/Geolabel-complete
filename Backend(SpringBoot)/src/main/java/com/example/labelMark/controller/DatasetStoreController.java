...
//            Integer taskId = (Integer) segmentationArr.get(i).get("task_id");
            ...
            // 动态获取坐标系
            String coordinateSystem = coordinateSystemUtils.getCoordinateSystemFromTasks(List.of(taskId));
            geoServerService.getGeoserverImg(taskService.getServerById(taskId), 256, 256, geoBbox, coordinateSystem);
...
//        String jsonString = mapper.writeValueAsString(json);
    ...
    public Result generateMergedDataset(@RequestBody Map<String, Object> map) throws IOException {
        // 1. 获取参数
        List<Integer> taskIds = (List<Integer>) map.get("taskIds");
        String datasetName = (String) map.get("datasetName"); // 用户自定义的数据集名称
        Integer currentUserId = Integer.valueOf(map.get("userId").toString());

        if (taskIds == null || taskIds.isEmpty()) {
            return ResultGenerator.getFailResult("未选择任何任务");
        }

        // 2. 准备数据集根目录 (使用 datasetName 而不是 taskId)
        // 注意：建议将 datasetName 进行去除非法字符处理
        String safeDatasetName = datasetName.replaceAll("[^a-zA-Z0-9_\\-]", "");
        Path outputDir = Paths.get(sampleSetPath + File.separator +safeDatasetName);
        Path outputDirImage = outputDir.resolve("images");
        Path outputDirAnnotations = outputDir.resolve("annotations");
        Path tempDir = Paths.get(sampleSetPath + File.separator+"dataset_temp"+ File.separator + safeDatasetName);

        createDirectory(outputDir, "创建数据集根目录");
        createDirectory(outputDirImage, "创建images目录");
        createDirectory(outputDirAnnotations, "创建annotations目录");
        createDirectoryIfNotExists(tempDir); // 用于存放切片

        // 3. 在数据库创建数据集记录 (逻辑需根据你的 datasetStoreService 调整)
        // 假设这里创建一个总记录，ID为 datasetId
        int datasetId = datasetStoreService.createDatasetWithName(0, currentUserId, datasetName);

        // 4. 初始化全局集合
        List<Map<String, Object>> allImages = new ArrayList<>();
        List<Map<String, Object>> allAnnotations = new ArrayList<>();
        // 使用 Map 来去重类别 (Name -> CategoryInfo)
        Map<String, Map<String, Object>> categoryMap = new HashMap<>();

        int globalImageId = 1; // 全局图片ID计数器
        int globalAnnotationId = 1; // 全局标注ID计数器

        // ================== 开始循环处理每个任务 ==================
        for (Integer taskId : taskIds) {
            System.out.println("开始处理任务 ID: " + taskId);

            // --- A. 获取任务基础信息 ---
            Task task = taskService.selectTaskById(taskId);
            if (task == null) continue;
            List<Mark> marks = markService.selectMarkById(taskId);
            if (marks.isEmpty()) continue; // 如果该图没有标注，跳过

            // --- B. 获取 GeoServer 信息 (代码复用原有逻辑) ---
            String layerInfo = geoServerRESTClient.getLayerInfo(taskService.getServerById(taskId));
            if (layerInfo.startsWith("ERROR")) continue;
            ...
            double height = 2048; // 固定高度
            double width = Math.ceil(((maxx - minx) / (maxy - miny)) * height);
            // 修正 bbox 顺序: minx, miny, maxx, maxy
            String bboxStr = String.format("%f,%f,%f,%f", minx, miny, maxx, maxy);

            // --- C. 下载大图 (底图) ---
            String fileName = "train_" + taskId + ".tif"; // 保持文件名唯一性

            // 只有当文件不存在时才下载，节省时间
            Path largeFilePath = outputDirImage.resolve(fileName);
            ...
            // 生成 Mask 图片
            Path stuffImgPath = outputDirImage.resolve("train_" + taskId + ".jpeg");
            GenerateStuffImg.generateStuffImg((int) Math.round(width), (int) Math.round(height), segmentationArr, stuffImgPath.toString());

            // --- E. 记录 Image 信息 ---
            int currentImageId = globalImageId++; // 分配并自增
            Map<String, Object> imageInfo = new HashMap<>();
            imageInfo.put("file_name", fileName); // 指向 TIF 或 JPEG，看你的训练需求
            imageInfo.put("id", currentImageId);
            imageInfo.put("width", width);
            imageInfo.put("height", height);
            imageInfo.put("original_task_id", taskId); // 记录来源方便追溯
            allImages.add(imageInfo);

            // --- F. 处理具体的标注 (Annotations) ---
            for (int i = 0; i < segmentationArr.size(); i++) {
                ...
                String typeName = typeService.getTypeNameById(typeId); // 建议缓存，不要循环查库
                String typeColor = (String) segItem.get("type_color");

                // 1. 处理类别 (Category) - 确保跨任务合并
                // 使用 typeName + typeId 作为 Key 防止重复
                if (!categoryMap.containsKey(typeName)) {
                    Map<String, Object> category = new HashMap<>();
                    category.put("supercategory", "object");
                    category.put("id", typeId); // 保持原有的 typeId，或者生成新的 ID
                    category.put("name", typeName);
                    category.put("color", typeColor);
                ...
                // 2. 处理切片 (Slice)
                String geoBbox = (String) segItem.get("geoBbox");

                // [修复 BUG] 这里要接收返回值
                // 动态获取坐标系
                String coordinateSystem = coordinateSystemUtils.getCoordinateSystemFromTasks(List.of(taskId));
                ResponseEntity<byte[]> sliceResult = geoServerService.getGeoserverImg(
                        taskService.getServerById(taskId), 256, 256, geoBbox, coordinateSystem
                );

                String sliceFileName = "slice_" + taskId + "_" + i + ".jpeg";
                Path sliceFilePath = tempDir.resolve(sliceFileName);

                try (FileOutputStream fos = new FileOutputStream(sliceFilePath.toFile())) {
                    if (sliceResult.getBody() != null) {
                        fos.write(sliceResult.getBody()); // [修复 BUG] 写入 sliceResult
                    }
                }

                // 插入切片记录到数据库 (关联到总数据集 ID)
                datasetStoreService.insertSampleImgInfo(datasetId, typeId, sliceFilePath.toString());

                // 3. 构建 COCO Annotation
                Map<String, Object> annotation = new HashMap<>();
                annotation.put("id", globalAnnotationId++);
                annotation.put("image_id", currentImageId); // 关联到上面分配的 currentImageId
                annotation.put("category_id", typeId);
                annotation.put("bbox", segItem.get("bbox"));
                annotation.put("segmentation", segItem.get("segmentation"));
                annotation.put("iscrowd", 0);
                annotation.put("area", calculateArea((List<Double>)segItem.get("segmentation"))); // 可选，建议计算多边形面积
        ...
        finalJson.put("images", allImages);
        finalJson.put("annotations", allAnnotations);
        finalJson.put("categories", new ArrayList<>(categoryMap.values()));
        finalJson.put("info", Map.of("description", datasetName, "date_created", new java.util.Date()));

        ObjectMapper mapper = new ObjectMapper();
        mapper.enable(SerializationFeature.INDENT_OUTPUT);
        try {
            File outputFile = outputDirAnnotations.resolve("annotations.json").toFile();
            mapper.writeValue(outputFile, finalJson);
            System.out.println("合并数据集生成成功: " + outputFile.getAbsolutePath());
        } catch (IOException e) {
            e.printStackTrace();
            return ResultGenerator.getFailResult("JSON写入失败");
        }

        return ResultGenerator.getSuccessResult("样本集生成成功，包含 " + taskIds.size() + " 个任务数据");
    }

    // 简单的辅助方法，用于计算多边形面积（鞋带公式），如果不重要可省略
    private double calculateArea(List<Double> segmentation) {
